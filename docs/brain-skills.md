...
| grill-decisions | decide | read | **write** | **write** | — | plan → operate-beads |
| build-context | describe | **write** | read | read | — | — |
| operate-beads | execute | read | read | read | **write** (conventions) | **write** |
| enter-main-agent | activate | read | read | read | read | claim/close |
| triage-public | feed | — | — | — | read | via operate-beads |
| review-architecture | reflect | read | read | read | — | findings → operate-beads |
...
to become ADRs and through `operate-beads` to become beads.
...
`GitHub intake:`); `operate-beads` owns conventions (e.g. `labels.md`). `triage-public` is a
