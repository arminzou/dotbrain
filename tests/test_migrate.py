...
def _seed_beads(brainspace: Path, mode: str | None) -> None:
...
def test_backup_dir_outside_beads(tmp_path: Path):
    brainspace = tmp_path / "brainspaces" / "demo"
    assert migrate.backup_dir_for(brainspace) == tmp_path / "backups" / "beads" / "demo"
...
def test_beads_mode(dotbrain_home: Path):
    embedded = paths.brainspace(dotbrain_home, "emb")
    _seed_beads(embedded, "embedded")
    server = paths.brainspace(dotbrain_home, "srv")
    _seed_beads(server, "server")
    nobeads = paths.brainspace(dotbrain_home, "bare")
    nobeads.mkdir(parents=True)
    unknown = paths.brainspace(dotbrain_home, "unk")
...
def test_migrate_embedded_happy_path_emits_full_argv_sequence(dotbrain_home: Path):
    ...
    backup = str(dotbrain_home / "backups" / "beads" / "example")
    init = [
    ...
    assert calls == [
        ...
        ["ssh", "-o", "BatchMode=yes", "-o", "ConnectTimeout=10",
         "10.0.0.1", "mkdir", "-p", str(dotbrain_home / "backups" / "beads")],
...
def test_migrate_database_override(dotbrain_home: Path):
    brainspace = paths.brainspace(dotbrain_home, "example")
    _seed_beads(brainspace, "embedded")
    calls: list[list[str]] = []
...
def test_migrate_dry_run_emits_no_bd_calls(dotbrain_home: Path):
    brainspace = paths.brainspace(dotbrain_home, "example")
    _seed_beads(brainspace, "embedded")
    calls: list[list[str]] = []
    ...
    assert not list(dotbrain_home.glob(".beads.wire-project.*"))


def test_migrate_count_mismatch_aborts_project(dotbrain_home: Path):
    brainspace = paths.brainspace(dotbrain_home, "example")
    _seed_beads(brainspace, "embedded")
    calls: list[list[str]] = []
...
def test_migrate_unverified_when_stats_unparseable(dotbrain_home: Path):
    brainspace = paths.brainspace(dotbrain_home, "example")
    _seed_beads(brainspace, "embedded")
    calls: list[list[str]] = []
    ...
    result = migrate.migrate_project(
        dotbrain_home=dotbrain_home, project="example", server_host="h", run=run,
    )
...
def test_migrate_skips_server_mode_idempotent(dotbrain_home: Path):
    brainspace = paths.brainspace(dotbrain_home, "example")
    _seed_beads(brainspace, "server")
    calls: list[list[str]] = []

    result = migrate.migrate_project(
        dotbrain_home=dotbrain_home, project="example", server_host="h",
        run=make_migrate_runner(calls),
    )
...
def test_migrate_skips_unknown_mode(dotbrain_home: Path):
    brainspace = paths.brainspace(dotbrain_home, "example")
    _seed_beads(brainspace, None)  # .beads dir but no metadata.json
    calls: list[list[str]] = []
...
def test_migrate_non_dotbrain_project_hides_root_beads(dotbrain_home: Path):
    # A repo-root .beads symlink that must be hidden while bd runs in a non-dotbrain Brainspace.
    real = dotbrain_home / "brainspaces" / "example" / ".beads"
    brainspace = paths.brainspace(dotbrain_home, "example")
    _seed_beads(brainspace, "embedded")
    (dotbrain_home / ".beads").symlink_to(real)
    calls: list[list[str]] = []
    ...
    result = migrate.migrate_project(
        dotbrain_home=dotbrain_home, project="example", server_host="h", run=spy,
    )
    assert result.status == "migrated"
    # symlink restored afterward, no leftover temp
    assert (dotbrain_home / ".beads").is_symlink()
    assert not list(dotbrain_home.glob(".beads.wire-project.*"))


def test_migrate_dotbrain_project_does_not_hide_root_beads(dotbrain_home: Path):
    brainspace = paths.brainspace(dotbrain_home, "dotbrain")
    _seed_beads(brainspace, "embedded")
    real = brainspace / ".beads"
    (dotbrain_home / ".beads").symlink_to(real)
    calls: list[list[str]] = []

    result = migrate.migrate_project(
        dotbrain_home=dotbrain_home, project="dotbrain", server_host="h",
        run=make_migrate_runner(calls),
    )
    assert result.status == "migrated"
...
def test_migrate_all_mixes_embedded_and_server(dotbrain_home: Path):
    _seed_beads(paths.brainspace(dotbrain_home, "emb"), "embedded")
    _seed_beads(paths.brainspace(dotbrain_home, "srv"), "server")
    paths.brainspace(dotbrain_home, "bare").mkdir(parents=True)
    calls: list[list[str]] = []

    results = migrate.migrate_all(
        dotbrain_home=dotbrain_home, server_host="h",
        run=make_migrate_runner(calls),
    )
...
def test_safe_migrate_project_reports_ssh_failure_cleanly(dotbrain_home: Path):
    ...
    result = migrate.safe_migrate_project(
        dotbrain_home=dotbrain_home, project="emb", server_host="h", run=failing_run,
    )
...
def test_migrate_abort_keeps_embedded_entry(dotbrain_home: Path):
    ...
    assert result.status == "aborted-count-mismatch"
    assert config.load_project_config(dotbrain_home, "example").mode == "embedded"
