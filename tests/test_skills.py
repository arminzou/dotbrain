...
def test_discover_skills_finds_all(dotbrain_root: Path):
    found = skills.discover_skills(dotbrain_root / "skills")
    assert found == [
        "brain/build-context",
...
def test_discover_skills_skips_node_modules(dotbrain_root: Path):
    nm = dotbrain_root / "skills" / "frontend" / "node_modules" / "pkg"
    nm.mkdir(parents=True)
    (nm / "SKILL.md").write_text("# temporary fixture\n")
    assert "frontend/node_modules/pkg" not in skills.discover_skills(dotbrain_root / "skills")
...
def test_link_into_links_an_include_list(dotbrain_root: Path, tmp_path: Path):
    ...
    result = skills.link_into(
        dotbrain_root, dest, ("brain/wire-brain", "misc/discovery-test"), label="codex"
    )
    assert not result.warnings
    assert (dest / "wire-brain").resolve() == (dotbrain_root / "skills" / "brain" / "wire-brain").resolve()
    assert (dest / "discovery-test").is_symlink()
    assert result.linked == ["codex/wire-brain", "codex/discovery-test"]


def test_link_into_owned_prune_keeps_foreign_links(dotbrain_root: Path, tmp_path: Path):
    dest = tmp_path / "global-skills"
    dest.mkdir()
    # a stale link into the dotbrain skills tree -> pruned; an unrelated link -> kept.
    (dest / "old").symlink_to(dotbrain_root / "skills" / "misc" / "discovery-test")
    (dest / "foreign").symlink_to(tmp_path / "elsewhere")
    skills.link_into(dotbrain_root, dest, ("brain/wire-brain",), prune_owned_only=True)
...
def test_link_into_stashes_real_collision(dotbrain_root: Path, tmp_path: Path):
    dest = tmp_path / "global-skills"
    dest.mkdir()
    real = dest / "wire-brain"
    real.write_text("real, do not delete")
    result = skills.link_into(dotbrain_root, dest, ("brain/wire-brain",))
    assert (dest / "wire-brain").is_symlink()
