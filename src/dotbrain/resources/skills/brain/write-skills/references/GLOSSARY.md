...
Terms below extend the base above with dotbrain's own skill-management model. They describe where a skill is _stored_ and _reached_ in the dotbrain data root, orthogonal to everything above (which is about a skill's internal shape).

### Bucket

A storage-only grouping of skills under `skills/<bucket>/` in the dotbrain data root (`brain/`, `engineering/`, `productivity/`, `personal/`, …). It organises the folder tree and nothing else: a bucket does not decide a skill's reach or invocation. `brain/` is the bucket that holds the **Brain-coupled skills**; every other bucket holds **personal skills**.

_Avoid_: category, namespace, scope (scope is the link surface, not the folder)
...
An operator-owned skill that ships with no force-wiring — the complement of a **Brain-coupled skill**. It is opt-in, reached only when the operator lists it: in `skills/skills.yaml` under `global_extra:` (linked into every session) or in a project's `project.yaml` under `skills:` (linked into one Brainspace). Lives in any **bucket** other than `brain/`.

_Avoid_: extra skill, user skill

### Linking

Reconciling a skill's declaration in config into agent-workspace symlinks: `dotbrain skills link --scope global` materialises the `global_extra:` list, `--scope project --project <name>` materialises one Brainspace's `project.yaml` `skills:` list, and `--scope all` does both. Idempotent — it creates missing links and prunes ones no longer selected. Agent-facing rule: **suggest** the command to the user, scoped to where the skill belongs; never run it automatically.

_Avoid_: install, wire (wiring is the repo↔Brainspace symlink set, not skill links)
